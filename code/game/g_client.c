...
void G_ResetTrail( gentity_t *ent ) {
	...
	for ( i = ent->client->trailHead, time = level.time; i >= 0; i--, time -= 50 ) {
		VectorCopy( ent->r.absmin, ent->client->trail[i].mins );
		VectorCopy( ent->r.absmax, ent->client->trail[i].maxs );
...
void G_StoreTrail( gentity_t *ent ) {
	...
	VectorCopy( ent->r.absmin, ent->client->trail[head].mins );
	VectorCopy( ent->r.absmax, ent->client->trail[head].maxs );
...
void G_TimeShiftClient( gentity_t *ent, int time ) {
	...
	if ( j != k ) {
		...
		if ( ent->client->saved.leveltime != level.time ) {
			...
			VectorCopy( ent->r.absmin, ent->client->saved.mins );
			VectorCopy( ent->r.absmax, ent->client->saved.maxs );
		...
		if ( j != ent->client->trailHead ) {
			...
			VectorCopy( ent->client->trail[k].mins, ent->r.absmin );
			VectorCopy( ent->client->trail[k].maxs, ent->r.absmax );
...
void G_UnTimeShiftClient( gentity_t *ent ) {
	...
	if ( ent->client->saved.leveltime == level.time ) {
		// move it back
		VectorCopy( ent->client->saved.mins, ent->r.absmin );
		VectorCopy( ent->client->saved.maxs, ent->r.absmax );
